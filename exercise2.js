...
function getServicesCost(services){
    return new Promise((resolve,reject) =>{
        setTimeout(function(){
            let cost=0;
            if(services.includes("service1")){
                cost+=150;
            }
            if(services.includes("service2")){
                cost+=120;
            }
            if(services.includes("service3")){
                cost+=300;
            }
            if(services.includes("service4")){
                cost+=500;
            }
            resolve(cost);
        },1000); 
...
// .then((totalcost)=>console.log(totalcost));
